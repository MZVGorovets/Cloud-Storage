...
class Message_Manipulations():
    ...
    def sending_file(self, message):
        SuperSocket(self.client_socket).send_msg(
            Encryption(self.key).encryption_file(message))

    def receiving_file(self):
        return Encryption(self.key).decryption_file(SuperSocket(self.client_socket).recv_msg())
